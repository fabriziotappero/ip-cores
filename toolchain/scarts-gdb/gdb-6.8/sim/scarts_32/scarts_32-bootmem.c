...
scarts_bootmem_init (void)
{
   ...
   *
   * Assembler code:
   * ldhi  r13, (SCARTS_CODEMEM_LMA >> 24) & 0xFF
   * ldliu r13, (SCARTS_CODEMEM_LMA >> 16) & 0xFF
   * sli r13,0x8
   * ldliu r13, (SCARTS_CODEMEM_LMA >> 8) & 0xFF
   * sli r13,0x8
   * ldliu r13, SCARTS_CODEMEM_LMA & 0xFF
   * jmp r13
   */

  bootmem[0] = (0x1 << 12) | ((SCARTS_CODEMEM_LMA >> 24) & 0xFF) << 4 | 0xD;
  bootmem[1] = (0x2 << 12) | ((SCARTS_CODEMEM_LMA >> 16) & 0xFF) << 4 | 0xD;
  bootmem[2] = 0xA18D;
  bootmem[3] = (0x2 << 12) | ((SCARTS_CODEMEM_LMA >> 8) & 0xFF) << 4 | 0xD;
  bootmem[4] = 0xA18D;
  bootmem[5] = (0x2 << 12) | (SCARTS_CODEMEM_LMA & 0xFF) << 4 | 0xD;
  bootmem[6] = 0xEF0D;
}

int
scarts_bootmem_read (uint32_t addr, uint16_t* value)
{
...
int
scarts_bootmem_write (uint32_t addr, uint16_t value)
{
  if (addr >= SCARTS_BOOTMEM_SIZE)
    return 0;
