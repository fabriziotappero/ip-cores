...
//		else				fprintf(stderr,"NO DSR(DTR)\n");
	...
	// command(4byte)
	{
		...
		tx(0x00); rx(0x00);
		tx(0x00); rx(0x00);
		tx(0x01); rx(0x01); // 0x00=nop 0x01=write 0x02=read 0x03=boot
	}
	...
	// write data
	{
		...
		size	= strtoul(argv[3],NULL,16);
		for (i=0;i<size;i++) {
			if (1 != read( fileno(stdin),&data,1) ) {
				fprintf(stderr,"\nEOF\n");
				break;
			}
			tx(data); rx(data); // write and ack
			fprintf(stderr,"\r(0x%08X)<=0x%02x",(unsigned int)address,data);
			address++;
		}
		fprintf(stderr,"\n");
	}
