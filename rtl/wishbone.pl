...
sub gen_adr_decoder{
  ...
  if ($interconnect eq "sharedbus") {
    ...
    # slave select
    for ($i=1; $i <= $slaves; $i++) {
	$hi_adrbit=$adr_size-1;
	$lo_adrbit=log(hex($slave[$i]{"size"}))/log(2);
      printf OUTFILE "%s_ss <= '1' when adr(%s downto %s)=\"",$slave[$i]{"wbs"}, $hi_adrbit,$lo_adrbit;
      $slave[$i]{"baseadr"}=hex($slave[$i]{"baseadr"});
      for ($j=$hi_adrbit; $j ne $lo_adrbit-1; $j--) {
        if (($slave[$i]{"baseadr"}) >= (2**$j)) {
          $slave[$i]{"baseadr"} -= 2**$j;
          printf OUTFILE "1"; # hharte - fixme
        } else {
          printf OUTFILE "0";
        };
      };
...
sub gen_muxshb{
    ...
    # rty
    if (($rty_o eq 0) && ($rty_i > 0)) {
      for ($i=1; $i <= $masters; $i++) {
        if ($master[$i]{"rty_i"} eq 1) {
          printf OUTFILE "%s_rty_i <= '0';\n",$master[$i]{"wbm"};
        };
      };
    } elsif (($rty_o eq 1) && ($rty_i > 0)) {
    ...
    # err
    if (($err_o eq 0) && ($err_i > 0)) {
      for ($i=1; $i <= $masters; $i++) {
        if ($master[$i]{"err_i"} eq 1) {
          printf OUTFILE "%s_err_i <= '0';\n",$master[$i]{"wbm"};
        };
      };
    } elsif (($err_o eq 1) && ($err_i > 0)) {
