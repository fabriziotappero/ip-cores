...
/// Basic constructor
posted_vc_l3::posted_vc_l3(sc_module_name name): sc_module(name)
{
	...
	for(int n = 0; n < NB_OF_BUFFERS;n++){
		sensitive << packet_addr_register[n]
				  ...
				  << packet_partofchain_register[n]
				  << clumped_unitid_registers[n]
				  << packet_nposted_refid_rejected_register[n]
				  << packet_response_refid_rejected_register[n]
				  << packet_nposted_refid_accepted_register[n]
				  << packet_response_refid_accepted_register[n]
#endif
	...
	/***********************************
		chainMarkerLinking
	***********************************/
	chainMarkerModule = new chain_marker_l4("chainMarkerModule");

	chainMarkerModule->clk(clk);
	chainMarkerModule->resetx(resetx);

	chainMarkerModule->in_packet_chain(in_packet_chain);
	for(int n = 0; n < 2; n++){
		chainMarkerModule->in_packet_destination[n](in_packet_destination[n]);
	}
	chainMarkerModule->input_packet_partofchain(input_packet_partofchain);
#endif
...
#ifdef SYSTEMC_SIM
	posted_vc_l3::~posted_vc_l3(){
		delete chainMarkerModule;
	}
#endif
...
*/
bool posted_vc_l3::evaluate_switch_packets(bool high_passPW,bool low_passPW,
					sc_uint<4> high_seqID, sc_uint<4> low_seqID,
					bool high_partofchain, bool low_partofchain,
					...
					bool compare_with_higher)
{
	...
	//This checks if the packet has more priority than previous packets in
	//the virtual channel
	bool select_low;
	if( 
		...
		//This is to make sure that we don't mix chains and not chains.
		(low_partofchain && !high_partofchain || high_partofchain) ||
...
void posted_vc_l3::clocked_process()
{
	if(!resetx.read()){
		///////////////////////////////////////////
		...
		for(int n = 0; n < NB_OF_BUFFERS; n++){
			...
			packet_partofchain_register[n] = false;
			packet_nposted_refid_rejected_register[n] = 0;
			packet_response_refid_rejected_register[n] = 0;
			packet_nposted_refid_accepted_register[n] = 0;
			packet_response_refid_accepted_register[n] = 0;
			pass_count[n] = 0;
#endif
		}
#ifdef ENABLE_REORDERING
...
void posted_vc_l3::select_and_drive_output(){
...
void posted_vc_l3::update_internal_registers()
{
	...
	*/
	...
	bool lower_passPW[NB_OF_BUFFERS/2];
	sc_uint<4> lower_seqID[NB_OF_BUFFERS/2];
	...
	for(int n = 0; n < NB_OF_BUFFERS/2; n++){
		if(unitid_reorder_disable.read()){
			...
			switch_packets[n] = evaluate_switch_packets(
				...
				packet_partofchain_register[2*n+1].read(),lower_packet_partofchain[n],
	...
	case 7:
		...
		packet_partofchain_register[0] = input_packet_partofchain;//From chain_marker
		packet_nposted_refid_rejected_register[0] = nposted_refid_rejected;
		packet_response_refid_rejected_register[0] = response_refid_rejected;
		packet_nposted_refid_accepted_register[0] = nposted_refid_accepted;
		packet_response_refid_accepted_register[0] = response_refid_accepted;
		pass_count[0] = 0;
		break;
	...
	case 4:
		...
		packet_partofchain_register[0] = packet_partofchain_register[1];
		packet_nposted_refid_rejected_register[0] = packet_nposted_refid_rejected_register[1];
		packet_response_refid_rejected_register[0] = packet_response_refid_rejected_register[1];
		packet_nposted_refid_accepted_register[0] = packet_nposted_refid_accepted_register[1];
		packet_response_refid_accepted_register[0] = packet_response_refid_accepted_register[1];
	...
	case 1:
	case 3:
	//case 4 removed : packet 0 switching with packet 1, take older refid value
	case 7:
	case 11:
	case 15:

		packet_nposted_refid_rejected_register[1] = packet_nposted_refid_rejected_register[0];
		packet_response_refid_rejected_register[1] = packet_response_refid_rejected_register[0];
		packet_nposted_refid_accepted_register[1] = packet_nposted_refid_accepted_register[0];
		packet_response_refid_accepted_register[1] = packet_response_refid_accepted_register[0];
		break;

	case 2:
	case 10:
		packet_nposted_refid_rejected_register[1] = packet_nposted_refid_rejected_register[2];
		packet_response_refid_rejected_register[1] = packet_response_refid_rejected_register[2];
		packet_nposted_refid_accepted_register[1] = packet_nposted_refid_accepted_register[2];
		packet_response_refid_accepted_register[1] = packet_response_refid_accepted_register[2];
		break;

	default:
		packet_nposted_refid_rejected_register[1] = packet_nposted_refid_rejected_register[1];
		packet_response_refid_rejected_register[1] = packet_response_refid_rejected_register[1];
		packet_nposted_refid_accepted_register[1] = packet_nposted_refid_accepted_register[1];
		packet_response_refid_accepted_register[1] = packet_response_refid_accepted_register[1];
	}
	...
	///Passing NB_OF_BUFFERS - 2, so take that refid value
	case 5:
		packet_nposted_refid_rejected_register[NB_OF_BUFFERS - 1] = packet_nposted_refid_rejected_register[NB_OF_BUFFERS - 2];
		packet_response_refid_rejected_register[NB_OF_BUFFERS - 1] = packet_response_refid_rejected_register[NB_OF_BUFFERS - 2];
		packet_nposted_refid_accepted_register[NB_OF_BUFFERS - 1] = packet_nposted_refid_accepted_register[NB_OF_BUFFERS - 2];
		packet_response_refid_accepted_register[NB_OF_BUFFERS - 1] = packet_response_refid_accepted_register[NB_OF_BUFFERS - 2];
		break;

	default:
		packet_nposted_refid_rejected_register[NB_OF_BUFFERS - 1] = packet_nposted_refid_rejected_register[NB_OF_BUFFERS - 1];
		packet_response_refid_rejected_register[NB_OF_BUFFERS - 1] = packet_response_refid_rejected_register[NB_OF_BUFFERS - 1];
		packet_nposted_refid_accepted_register[NB_OF_BUFFERS - 1] = packet_nposted_refid_accepted_register[NB_OF_BUFFERS - 1];
		packet_response_refid_accepted_register[NB_OF_BUFFERS - 1] = packet_response_refid_accepted_register[NB_OF_BUFFERS - 1];
	}
	...
	for(int n = 0; n < NB_OF_BUFFERS/2 - 1; n++){
		...
		///Passing 2 * n + 2, so take that refid value
		case 12:
		case 13:
			packet_nposted_refid_rejected_register[2 * n + 3] = packet_nposted_refid_rejected_register[2 * n + 2];
			packet_response_refid_rejected_register[2 * n + 3] = packet_response_refid_rejected_register[2 * n + 2];
			packet_nposted_refid_accepted_register[2 * n + 3] = packet_nposted_refid_accepted_register[2 * n + 2];
			packet_response_refid_accepted_register[2 * n + 3] = packet_response_refid_accepted_register[2 * n + 2];
			break;

		case 8:
		case 10:
			packet_nposted_refid_rejected_register[2 * n + 3] = packet_nposted_refid_rejected_register[2 * n + 4];
			packet_response_refid_rejected_register[2 * n + 3] = packet_response_refid_rejected_register[2 * n + 4];
			packet_nposted_refid_accepted_register[2 * n + 3] = packet_nposted_refid_accepted_register[2 * n + 4];
			packet_response_refid_accepted_register[2 * n + 3] = packet_response_refid_accepted_register[2 * n + 4];
			break;

		default:
			packet_nposted_refid_rejected_register[2 * n + 3] = packet_nposted_refid_rejected_register[2 * n + 3];
			packet_response_refid_rejected_register[2 * n + 3] = packet_response_refid_rejected_register[2 * n + 3];
			packet_nposted_refid_accepted_register[2 * n + 3] = packet_nposted_refid_accepted_register[2 * n + 3];
			packet_response_refid_accepted_register[2 * n + 3] = packet_response_refid_accepted_register[2 * n + 3];
		}
		...
		case 4:
		case 5:
		//case 8 and 9 remove, 2 * n + 2 switching with 2 * n + 1, take older value
		case 14:
		case 15:
		//case 6 and 7, move_up and switch, take older value
		case 6:
		case 7:
			packet_nposted_refid_rejected_register[2 * n + 2] = packet_nposted_refid_rejected_register[2 * n + 1];
			packet_response_refid_rejected_register[2 * n + 2] = packet_response_refid_rejected_register[2 * n + 1];
			packet_nposted_refid_accepted_register[2 * n + 2] = packet_nposted_refid_accepted_register[2 * n + 1];
			packet_response_refid_accepted_register[2 * n + 2] = packet_response_refid_accepted_register[2 * n + 1];
			break;

		case 1:
		case 3:
			packet_nposted_refid_rejected_register[2 * n + 2] = packet_nposted_refid_rejected_register[2 * n + 3];
			packet_response_refid_rejected_register[2 * n + 2] = packet_response_refid_rejected_register[2 * n + 3];
			packet_nposted_refid_accepted_register[2 * n + 2] = packet_nposted_refid_accepted_register[2 * n + 3];
			packet_response_refid_accepted_register[2 * n + 2] = packet_response_refid_accepted_register[2 * n + 3];
			break;

		default:
			packet_nposted_refid_rejected_register[2 * n + 2] = packet_nposted_refid_rejected_register[2 * n + 2];
			packet_response_refid_rejected_register[2 * n + 2] = packet_response_refid_rejected_register[2 * n + 2];
			packet_nposted_refid_accepted_register[2 * n + 2] = packet_nposted_refid_accepted_register[2 * n + 2];
			packet_response_refid_accepted_register[2 * n + 2] = packet_response_refid_accepted_register[2 * n + 2];
		}
...
void posted_vc_l3::drive_output(sc_bv<NB_OF_BUFFERS> selected_accepted,
								bool selected_accepted_found,
								...
								bool selected_forward_found)
{
	...
	bool out_packet_chain_accepted = false;
	sc_uint<LOG2_NB_OF_BUFFERS+1>	out_packet_nposted_refid_accepted = 0;
	sc_uint<LOG2_NB_OF_BUFFERS+1>	out_packet_response_refid_accepted = 0;
#endif
	...
	for(int n = 0; n < NB_OF_BUFFERS; n++){
		...
		bool  select_out_packet_chain;
		sc_uint<LOG2_NB_OF_BUFFERS+1>	select_out_packet_nposted_refid;
		sc_uint<LOG2_NB_OF_BUFFERS+1>	select_out_packet_response_refid;
#endif
		...
		select_out_packet_chain = packet_partofchain_register[n].read() && (sc_bit)selected_accepted[n];
		for(int i = 0; i < LOG2_NB_OF_BUFFERS+1; i++){
			select_out_packet_nposted_refid[i] = (sc_bit)packet_nposted_refid_accepted_register[n].read()[i] && (sc_bit)selected_accepted[n];
			select_out_packet_response_refid[i] = (sc_bit)packet_response_refid_accepted_register[n].read()[i] && (sc_bit)selected_accepted[n];
		}
#endif
		...
		out_packet_chain_accepted = out_packet_chain_accepted || select_out_packet_chain;
		out_packet_nposted_refid_accepted = out_packet_nposted_refid_accepted || select_out_packet_nposted_refid;
		out_packet_response_refid_accepted = out_packet_response_refid_accepted || select_out_packet_response_refid;
#endif
	}
	...
	out_packet_chain[ACCEPTED_DEST] = out_packet_chain_accepted;
	out_packet_nposted_refid[ACCEPTED_DEST] = out_packet_nposted_refid_accepted;
	out_packet_response_refid[ACCEPTED_DEST] = out_packet_response_refid_accepted;
#endif
	...
	bool out_packet_chain_forward = false;
	sc_uint<LOG2_NB_OF_BUFFERS+1>	out_packet_nposted_refid_forward = 0;
	sc_uint<LOG2_NB_OF_BUFFERS+1>	out_packet_response_refid_forward = 0;
#endif
	...
	for(int n = 0; n < NB_OF_BUFFERS; n++){
		...
		select_out_packet_chain = packet_partofchain_register[n].read() && (sc_bit)selected_forward[n];
		for(int i = 0; i < LOG2_NB_OF_BUFFERS+1; i++){
			select_out_packet_nposted_refid[i] = (sc_bit)packet_nposted_refid_rejected_register[n].read()[i] && (sc_bit)selected_forward[n];
			select_out_packet_response_refid[i] = (sc_bit)packet_response_refid_rejected_register[n].read()[i] && (sc_bit)selected_forward[n];
		}
#endif
		...
		out_packet_chain_forward = out_packet_chain_forward || select_out_packet_chain;
		out_packet_nposted_refid_forward = out_packet_nposted_refid_forward || select_out_packet_nposted_refid;
		out_packet_response_refid_forward = out_packet_response_refid_forward || select_out_packet_response_refid;
#endif
	}
	...
	out_packet_chain[FWD_DEST] = out_packet_chain_forward;
	out_packet_nposted_refid[FWD_DEST] = out_packet_nposted_refid_forward;
	out_packet_response_refid[FWD_DEST] = out_packet_response_refid_forward;
#endif
...
void posted_vc_l3::select_top_packet(sc_bv<NB_OF_BUFFERS> &selected_accepted,
								bool &selected_accepted_found,
								sc_bv<NB_OF_BUFFERS> &selected_forward,
								bool &selected_forward_found)
{
