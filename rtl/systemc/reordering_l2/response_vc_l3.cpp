...
/// Basic constructor
response_vc_l3::response_vc_l3(sc_module_name name): sc_module(name)
{
...
*/
bool response_vc_l3::evaluate_switch_packets(
							bool high_passPW,
							bool low_passPW,
							bool high_buffer_free,
							bool low_buffer_free,
							sc_uint<MAX_PASSPW_P1_LOG2_COUNT> high_pass_count,
							sc_uint<MAX_PASSPW_P1_LOG2_COUNT> low_pass_count,
							/*Normally, this function should make abstraction that the
							high_pkt_complete can actually be lower, but when two packets
							have the same priority, we don't want them to be switched
							so we return the compare_with_higher value as an answer*/
							bool compare_with_higher)
{
	...
	if(high_pass_count == MAX_PASSPW_COUNT || low_pass_count == MAX_PASSPW_COUNT){
...
void response_vc_l3::clocked_process()
{
	if(!resetx.read()){
		///////////////////////////////////////////
...
void response_vc_l3::select_and_drive_output(){
...
void response_vc_l3::update_internal_registers()
{
...
void response_vc_l3::drive_output(sc_bv<NB_OF_BUFFERS> selected_accepted,
								bool selected_accepted_found,
...
void response_vc_l3::select_top_packet(sc_bv<NB_OF_BUFFERS> &selected_accepted,
								bool &selected_accepted_found,
								sc_bv<NB_OF_BUFFERS> &selected_forward,
								bool &selected_forward_found)
{
