...
#ifndef POSTED_VC_L3_H
#define POSTED_VC_L3_H

#include "../core_synth/synth_datatypes.h"
#include "../core_synth/constants.h"

//Forward declaration
class chain_marker_l4;

/// Request Virtual Channel
/** Used for Posted Command (PC)
	This module stores and reorganizes posted packets.
	The most important/recent packet is sent to the output buffer when
	it is free.
*/
class posted_vc_l3: public sc_module
{
public:
	/// Signal always at bool(0)
	sc_signal<bool> GND;
	...
	///The number of buffers that have been freed
	...
	sc_out<bool > out_packet_chain[2];
	sc_out<bool > out_packet_passpw[2];
	sc_out<sc_uint<4> > out_packet_seqid[2];
	sc_out<sc_uint<LOG2_NB_OF_BUFFERS+1> >	out_packet_nposted_refid[2];
	sc_out<sc_uint<LOG2_NB_OF_BUFFERS+1> >	out_packet_response_refid[2];
#endif
	...
	sc_in<sc_uint<4> > in_packet_seqid;
	sc_in<bool > in_packet_chain;
	sc_in<sc_uint<LOG2_NB_OF_BUFFERS+1> >	nposted_refid_rejected;
	sc_in<sc_uint<LOG2_NB_OF_BUFFERS+1> >	response_refid_rejected;
	sc_in<sc_uint<LOG2_NB_OF_BUFFERS+1> >	nposted_refid_accepted;
	sc_in<sc_uint<LOG2_NB_OF_BUFFERS+1> >	response_refid_accepted;
#endif
...
#ifdef ENABLE_REORDERING
	sc_in<sc_uint<5> > in_packet_clumped_unitid;
	...
	/// Marks packets if they are part of a chain
	chain_marker_l4*		chainMarkerModule;
...
#ifdef ENABLE_REORDERING
	///Packet fields necessary to evaluate reorganisation
	/** 
	   Other fields of the the packet are stored in a memory.
	   
	   Refids are numbers that track the arrival of packets relative to 
	   responses and nposted packets.  Refids need to be stored here because
	   when a packet with passPW passes a packet without passPW, if may have a
	   greater refid that could allow nposted or response packets that arrived
	   after the packet that is being passed to go through.  So when a packet
	   passes another packet, it takes the refid of the packet being passed.
	*/
	///@{
	sc_signal<bool > packet_passpw_register[NB_OF_BUFFERS];
	sc_signal<sc_uint<4> > packet_seqid_register[NB_OF_BUFFERS];
	sc_signal<bool > packet_partofchain_register[NB_OF_BUFFERS];
	sc_signal<sc_uint<LOG2_NB_OF_BUFFERS+1> >	packet_nposted_refid_rejected_register[NB_OF_BUFFERS];
	sc_signal<sc_uint<LOG2_NB_OF_BUFFERS+1> >	packet_response_refid_rejected_register[NB_OF_BUFFERS];
	sc_signal<sc_uint<LOG2_NB_OF_BUFFERS+1> >	packet_nposted_refid_accepted_register[NB_OF_BUFFERS];
	sc_signal<sc_uint<LOG2_NB_OF_BUFFERS+1> >	packet_response_refid_accepted_register[NB_OF_BUFFERS];
	///@}
	...
	///Buffers containing the clumped unitID of a packet 
	sc_signal<sc_uint<5> > clumped_unitid_registers[NB_OF_BUFFERS];
	...
	*/
	...
	/// Output signals from the Chain_Marker Module
	sc_signal<bool> input_packet_partofchain;
#endif
...
public:
	...
	*/
	...
	/**
		@param selected_accepted The packet selected to be output to accepted destination
			position is one-hot encoded (return by address)
		@param selected_accepted_found A packet was found to be output to accepted destination
			(return by address)
		@param selected_forward The packet selected to be output to forward destination
			(position is one-hot encoded  return by address)
	...
	*/
	...
	/// Basic constructor
	posted_vc_l3(sc_module_name name);

#ifdef SYSTEMC_SIM
	///Destructor
	virtual ~posted_vc_l3();
#endif
