...
cd_cmd_buffer_l3::cd_cmd_buffer_l3(sc_module_name name) : sc_module(name)
{
	SC_METHOD(bufferize);
	sensitive_pos(clk);
...
void cd_cmd_buffer_l3::bufferize()
{
	syn_ControlPacketComplete bufferPacket = packet.read();
